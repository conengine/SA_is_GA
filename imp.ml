...
let rec parsePrimaryExp steps xs =
  (fun zero succ n ->
    ...
    | SomeE p -> let Pair (i, rest) = p in SomeE (Pair ((AId0 i), rest))
    | NoneE _ ->
      (match parseNumber xs with
       | SomeE p ->
         let Pair (n0, rest) = p in SomeE (Pair ((ANum0 n0), rest))
       | NoneE _ ->
    ...
    | SomeE p ->
      ...
      (match many
       ...
       | SomeE p0 ->
         ...
         SomeE (Pair ((fold_left (fun x x0 -> AMult0 (x, x0)) es e), rest'))
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
      ...
      (match many (fun xs0 ->
       ...
       | SomeE p0 ->
         ...
         ((fold_left (fun e0 term ->
            ...
            if y then APlus0 (e0, e1) else AMinus0 (e0, e1)) es e), rest'))
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
                ...
                | SomeE p ->
                  ...
                  (match firstExpect (String ('=', (String ('=',
                   ...
                   | SomeE p0 ->
                     ...
                     SomeE (Pair ((BEq0 (e, e')), rest'))
                   | NoneE _ ->
                     (match firstExpect (String ('<', (String ('=',
                              EmptyString)))) (parseAExp steps') rest with
                      | SomeE p0 ->
                        let Pair (e', rest') = p0 in
                        SomeE (Pair ((BLe0 (e, e')), rest'))
                      | NoneE _ ->
  ...
  (fun zero succ n ->
    ...
    | SomeE p ->
      ...
      (match many
       ...
       | SomeE p0 ->
         let Pair (es, rest') = p0 in
         SomeE (Pair ((fold_left (fun x x0 -> BAnd0 (x, x0)) es e), rest'))
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
